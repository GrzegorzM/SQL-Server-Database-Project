...
Group By CAST(RegisteredDate as DATE); -- Grouped by date and time, so each group has exactly one result

Select CAST(RegisteredDate as DATE) as RegistrationDate, COUNT(Id) as TotalRegistrations From tblRegistrations
Group By CAST(RegisteredDate as DATE); -- Grouped by date, groups have different results